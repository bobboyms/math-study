...
lição, exercícios e gabarito — estão na skill de projeto `tutor-matematica`**
(`.agents/skills/tutor-matematica/`). Use-a antes de escrever, revisar ou
explicar qualquer conteúdo de matemática. Antes de tocar numa página de
`docs/`, leia também `references/escrever-licao.md` dentro dessa skill. Este
arquivo cobre apenas o funcionamento técnico do repositório.
...
git). A correção dessas imagens é o trabalho da skill de projeto `corrigir`
(`.agents/skills/corrigir/`): ela lê a imagem mais recente, confere cada conta
com `python3` e responde no papel de tutor. Não edite nem apague os arquivos
do caderno — são o registro do aluno.
